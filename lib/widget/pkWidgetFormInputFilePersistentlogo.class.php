 ...
 */
class pkWidgetFormInputFilePersistentlogo extends sfWidgetForm {
    ...
    public function render($name, $value = null, $attributes = array(), $errors = array()) {
        ...
        if (preg_match('/user_company_case_study_request/i', $name) == 1) {
            preg_match('/user_company_case_study_request\[logo]/', $name, $matches);
            $pwname = 'user_company_case_study_request';
        } else if (preg_match('/user_product_case_study_request/i', $name) == 1) {
            preg_match('/user_product_case_study_request\[logo]/', $name, $matches);
        ...
        if ($exists) {
            ...
            $result = '<span id="filename_uploaded'.'" style="padding-left:90px;"><input type="checkbox" name="'.$pwname.'[logo'.'][status]" id="'.$pwname.'_logo' . '_status" value="1" checked="true" /> ' .
                    '<label for="' . $pwname . '_logo' . '_status">' . $info['name'] . ' (' . sprintf('%.2f KB', $info['size'] / 1000) . ')</label></span><br/>';
            return
                    '<input type="hidden" name="' . $pwname . '[logo' . '][status]" id="' . $pwname . '_logo' . '_status" value="true" /> ' .
                    '<span id="filename_uploaded' . '" style="display: inline-block;width:395px;">' .
                    '<label for="' . $pwname . '_logo' . '_status" style="width:300px;color:#000;line-height: 16px;"><strong>' . sprintf('%s (%.2f KB)', $filename, $info['size'] / 1024) . '</strong></label></span><br/>' .
                    '<span id="file_' . '">' .
            ...
            $result = '<input type="hidden" name="' . $pwname . '[logo' . '][status]" id="' . $pwname . '_logo' . '_status" value="true" /> ';
        }
        ...
        if ($exists) {
            ...
            $result = '<span id="filename_uploaded' . $matches[1] . '" style="padding-left:90px;"><input type="checkbox" name="'.$pwname.'[logo' . $matches[1] . '][status]" id="'.$pwname.'_logo' . $matches[1] . '_status" value="1" checked="true" /> ' .
                    '<label for="' . $pwname . '_logo' . $matches[1] . '_status">' . $info['name'] . ' (' . sprintf('%.2f KB', $info['size'] / 1000) . ')</label></span><br/>';
            return
                    '<input type="hidden" name="' . $pwname . '[logo' . $matches[1] . '][status]" id="' . $pwname . '_logo' . $matches[1] . '_status" value="true" /> ' .
                    '<span id="filename_uploaded' . $matches[1] . '" style="display: inline-block;width:274px;">' .
                    '<label for="' . $pwname . '_logo' . $matches[1] . '_status"><strong>' . sprintf('%s (%.2f KB)', $filename, $info['size'] / 1024) . '</strong></label></span><br/>' .
            ...
            $result = '<input type="hidden" name="' . $pwname . '[logo' . $matches[1] . '][status]" id="' . $pwname . '_logo' . $matches[1] . '_status" value="true" /> ';
        } */
