 ...
 */
class pkWidgetFormInputFilePersistentContribucion extends sfWidgetForm {
    ...
    public function render($name, $value = null, $attributes = array(), $errors = array()) {
        ...
        preg_match('/contribucion\[archivo_(\d)\]\[file\]/', $name, $matches);
        $result = '';
        if ($exists) {
            ...
            return
                    '<input type="hidden" name="contribucion[archivo_' . $matches[1] . '][status]" id="contribucion_archivo_' . $matches[1] . '_status" value="true" /> ' .
                    '<span id="filename_uploaded' . $matches[1] . '" style="display: inline-block;width:274px;">' .
                    '<label for="contribucion_archivo_' . $matches[1] . '_status">' . sprintf('%s (%.2f KB)', $filename, $info['size'] / 1024) . '</label></span>' .
            ...
            $result = '<input type="hidden" name="contribucion[archivo_' . $matches[1] . '][status]" id="contribucion_archivo_' . $matches[1] . '_status" value="true" /> ';
        }
        if($value){
