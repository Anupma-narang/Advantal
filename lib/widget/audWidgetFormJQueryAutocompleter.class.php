...
/**
 * audWidgetFormJQueryAutocompleter represents an autocompleter input widget rendered by JQuery adding options to
 * delete and add values.
 *
 ...
 * @subpackage widget
 */
class audWidgetFormJQueryAutocompleter extends sfWidgetFormInput {
    ...
    public function render($name, $value = null, $attributes = array(), $errors = array()) {
        ...
        return $this->renderTag('input', array('type' => 'hidden', 'name' => $name, 'value' => $value)) .
                parent::render('autocomplete_' . $name, $visibleValue, array_merge($attributes, array('rel' => $this->getOption('add_url'))), $errors) .
                sprintf(<<<EOF
<script type="text/javascript">
  jQuery(document).ready(function() {
             var tipo = $('#lista_cuestionario_tipo').val();
    jQuery("#%s")
    .autocomplete('%s&tipo='+tipo, jQuery.extend({}, {
      dataType: 'json',
      parse:    function(data) {
        var parsed = [];
        for (key in data) {
...
EOF
                        , $this->generateId('autocomplete_' . $name), $this->getOption('url'), $this->getOption('config'), $this->generateId($name)
        );
    }
    ...
    public function getJavascripts() {
        return array('/sfFormExtraPlugin/js/jquery.autocompleter.js', '/js/audAutocomplete.js');
    }
