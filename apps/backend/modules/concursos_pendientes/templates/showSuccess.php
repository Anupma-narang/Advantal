...
<div id="sf_admin_container">
    <h1>Detalle del concurso pendiente</h1>

    <div id="sf_admin_content">
        <?php include_partial('concursos_pendientes/flashes') ?>

        <div id="sf_admin_header">
            <?php include_partial('concursos_pendientes/list_header') ?>
        </div>
        ...
        <ul class="dragbox-content">
            <li><span class="bold">Fecha</span>: <?php echo format_datetime($concurso->getCreatedAt(), "HH:mm dd/MM/y", "es_ES") ?></li>
            <?php if ($concurso->getFechaRevision()): ?>
                <li><span class="bold">Fecha de revisión</span>: <?php echo format_datetime($concurso->fecha_revision, "HH:mm dd/MM/y", "es_ES") ?></li>
            <?php endif; ?>

            <?php if ($concurso->getFechaActivacion()): ?>
                <li><span class="bold">Fecha de activación</span>: <?php echo format_datetime($concurso->fecha_activacion, "HH:mm dd/MM/y", "es_ES") ?></li>
            <?php endif; ?>

            <?php if ($concurso->getFechaReferendum()): ?>
                <li><span class="bold">Fecha de Referéndum</span>: <?php echo format_datetime($concurso->fecha_referendum, "HH:mm dd/MM/y", "es_ES") ?></li>
            <?php endif; ?>

            <?php if ($concurso->getFechaDeliberacion()): ?>
                <li><span class="bold">Fecha de Deliberación</span>: <?php echo format_datetime($concurso->fecha_deliberacion, "HH:mm dd/MM/y", "es_ES") ?></li>
            <?php endif; ?>

            <?php if ($concurso->getFechaCerrado()): ?>
                <li><span class="bold">Fecha cerrado</span>: <?php echo format_datetime($concurso->fecha_cerrado, "HH:mm dd/MM/y", "es_ES") ?></li>
            <?php endif; ?>

            <?php if ($concurso->getFechaRechazado()): ?>
                <li><span class="bold">Fecha rechazado</span>: <?php echo format_datetime($concurso->fecha_rechazado, "HH:mm dd/MM/y", "es_ES") ?></li>
            <?php endif; ?>

            <?php if ($concurso->getFechaNulo()): ?>
                <li><span class="bold">Fecha anulado</span>: <?php echo format_datetime($concurso->fecha_nulo, "HH:mm dd/MM/y", "es_ES") ?></li>
            <?php endif; ?>

            <li><span class="bold">Estado</span>: <?php echo $concurso->ConcursoEstado->name ?></li>
            <li><span class="bold">Usuario</span>: <?php echo $concurso->User ?></li>
            <li><span class="bold">Tipo de concurso</span>: <?php echo $concurso->ConcursoTipo->name ?></li>
            <li><span class="bold">Categoría</span>: <?php echo $concurso->ConcursoCategoria->name ?></li>

            <?php if ($concurso->ConcursoTipo->id == 1): ?>
                <li><span class="bold">Empresa/Entidad</span>: <?php echo $concurso->Empresa->name ?></li>
                <?php if (($concurso->States->name != "Toda España" && $concurso->getEmpresa()->getRoadTypeId()) || ($concurso->States->name == "Toda España" && $concurso->getEmpresa()->getRoadTypeId())): ?>
                    <li><span class="bold">Tipo de vía</span>: <?php echo $concurso->Empresa->RoadType->name ?></li>
                <?php endif; ?>
                <?php if (($concurso->States->name != "Toda España" && $concurso->concurso_address) || ($concurso->States->name == "Toda España" && $concurso->concurso_address)): ?>
                    <li><span class="bold">Dirección</span>: <?php echo $concurso->concurso_address ?><?php echo $concurso->concurso_numero ? ', Nº: ' . $concurso->concurso_numero : '' ?><?php echo $concurso->concurso_piso ? ', Piso: ' . $concurso->concurso_piso : '' ?><?php echo $concurso->concurso_puerta ? ', Puerta: ' . $concurso->concurso_puerta : '' ?></li>
                <?php endif; ?>
                <li><span class="bold">Provincia</span>: <?php echo $concurso->States->name ?></li>
                <li><span class="bold">Localidad</span>: <?php echo $concurso->getCity()->getName() ?></li>
                <?php if ($concurso->getEmpresa()->getCodigopostal()): ?>
                    <li><span class="bold">C.P.</span>: <?php echo $concurso->getEmpresa()->getCodigopostal() ?></li>
                <?php endif; ?>
                <li><span class="bold">Sector</span>: <?php echo $concurso->getEmpresa()->getSector(); ?></li>
                <li><span class="bold">Subsector</span>: <?php echo $concurso->getEmpresa()->getSubSector(); ?></li>
            ...
            <?php elseif ($concurso->ConcursoTipo->id == 2): ?>
                <li><span class="bold">Producto</span>: <?php echo $concurso->Producto->name ?></li>
                <li><span class="bold">Marca</span>: <?php echo $concurso->getProducto()->getMarca() ?></li>
                <?php if ($concurso->getProducto()->getModelo()): ?>
                    <li><span class="bold">Modelo</span>: <?php echo $concurso->getProducto()->getModelo() ?></li>
                    <li><span class="bold">Sector del producto</span>: <?php echo $concurso->getProducto()->getSectorProduct(); ?></li>
                    <li><span class="bold">Subsector del producto</span>: <?php echo $concurso->getProducto()->getSubSectorProduct(); ?></li>
                <?php endif; ?>
                <?php if ($concurso->getProducto()->getProductoTipoTresId()): ?>
                    <li><strong>Tipo de producto:</strong> <?php echo $concurso->getProducto()->getProductoTipoTres()->getName() ?></li>
                <?php endif; ?>
            <?php endif; ?>
            ...
            <li><span class="bold">Resumen del Plan de acción</span>: <p class="mr-span"> </p>
                <?php echo $contribucion_principal ? html_entity_decode($contribucion_principal->resumen) : '' ?>
                <div style="clear: both; height: 16px;"></div>
                <span class="ver_link">
                    <?php echo $contribucion_principal ? link_to('ver +', 'concursos_pendientes/showResumenPlanAccion?id=' . $contribucion_principal->getId(), array("popup" => array("popWindow", "width=650,height=500, left=200, scrollbars=1, menubar=1"))) : '' ?>
                </span>
            </li>
                    ...
                    <form id="Puntos_form" action="<?php echo url_for('concursos_pendientes/changeStatus?id=' . $concurso->getId()) ?>" method="get">
                        ...
                        <table>
                            <tbody>
                                ...
                                <tr>
                                    <td></td>
                                    <td ><strong><b><?php echo __('Tipo de puntos') ?></b></strong></td>
                                ...
                                <tr>
                                    <th>Otro</th>
                                    <td><input type="text" name="otro_descripcion" vsize="40" id="Otro_descripcion"></td>
                        ...
                        <div class="concurso_contribucion">
                            ...
                            <ul>
                                ...
                                /*
                                  ...
                                  <p class="mr-span"> </p>
                                  <?php echo truncate_text(html_entity_decode($contribucion->incidencia), 400) ?>
                                  <span class="ver_link">
                                  <?php if (strlen(html_entity_decode($contribucion->incidencia)) > 400) echo '<br/>' . link_to('ver +', 'contribucion/showIncidencia?id=' . $contribucion->getId(), array("popup" => array("popWindow", "width=650,height=500, left=200, scrollbars=1, menubar=1"))) ?>
                                  </span>
                                  </li>
                                 */
                                ?>
