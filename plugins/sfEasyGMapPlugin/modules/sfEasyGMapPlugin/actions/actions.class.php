...
class sfEasyGMapPluginActions extends sfActions
{
  ...
  public function executeIndex()
  {
    $this->available_samples = array(
      ...
      '11' => array('url' => 'sfEasyGMapPlugin/sample11', 'name' => 'Sample 11', 'message' => 'Save moving map'),
    );
  }
  ...
  public function executeSample2()
  {
    ...
    // Add event on marker
    $marker->addEvent(new GMapEvent('click', 'gmapSample_AddConsoleLine("getBounds"+getBounds())'));
  ...
  public function executeSample7()
  {
    ...
    $sample_address = 'APTIC, Barcelona, Spain';
  ...
  public function executeSample11($request)
  {
     // Initialize the google map
    $gMap = new GMap();
    $gMap->setZoom(19);
    $gMap->setCenter(41.46818625370226, 2.2851456461639508);
    // Create marker
    $marker = new GMapMarker(38.69408528314942, -0.8679197968749897, array('title' => '"Hello World !"','draggable' => "true"),'marker');

    // Add event on marker
//    $marker->addEvent(new GMapEvent('click', 'gmapSample_AddConsoleLine("getBounds"+getBounds())'));
    $marker->addEvent(new GMapEvent('dragend', 'gmapSample_AddConsoleLine("Position"+this.getPosition())'));

    // Add marker on the map
    $gMap->addMarker($marker);

    // Add event on the map
    $gMap->addEvent(new GMapEvent('click', 'gmapSample_AddConsoleLine(map.getBounds())'));
  ...
  public function executeUpdateMap(sfWebRequest $request)
  {
    $latlng = explode(',',trim($request->getParameter('bounds'),'\'()'));
    $marker_latlng = explode(',',trim($request->getParameter('marker_bounds'),"'()"));
    $gm = Doctrine_Core::getTable('google_map')->findOneBy('anunciant_id',$request->getParameter('anunciant_id'));
    if (!$gm) {
      $gm = new google_map();
      $gm->setAnunciantId($request->getParameter('anunciant_id'));
    }
    $gm->setLat($latlng[0]);
    $gm->setLng($latlng[1]);
    $gm->setMarkerLat($marker_latlng[0]);
    $gm->setMarkerLng($marker_latlng[1]);
    $gm->setMapTypeId($request->getParameter('map_type_id'));
            $gm->setZoom($request->getParameter('zoom'));
    $gm->save();
    return $this->renderText('ok');

  }

  public function executeUpdateDirections(sfWebRequest $request)
  {
    //goecode
    $gmap =  $gMap = new GMap();
    $geocoded_address = new GMapGeocodedAddress($request->getParameter('from'));
    return $this->renderText($geocoded_address->geocode($gmap));
//     return $this->renderText(json_encode(array($geocoded_address->getLat(), $geocoded_address->getLng())));
  }
}